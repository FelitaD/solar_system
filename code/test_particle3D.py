...
def test_init():
    ...
    assert isinstance(p.position, np.ndarray), "Particle positon not converted into an array from a list" + str(type(p.position))
...
def test_read_line():
    ...
    assert p_list is not None, "read_file did not return a value"
    assert len(p_list) == 3, "read_file read the wrong number of particles"
    ...
    assert np.isclose(p_list[0].mass, 1.0), "First particle mass not correct"
    assert np.isclose(p_list[1].mass, 0.5), "Second particle mass not correct"
    assert np.isclose(p_list[2].mass, -1.0), "Third particle mass not correct"
